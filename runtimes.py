    ...
    if requested in (None, "static-web") and has_html:
        ...
        return RuntimeAdapter(
            ...
            verifier_guidance=(
                ...
                "and node:assert against the existing `document`/`window`. "
                "IMPORTANT: `window` is a plain simulated object, not the real global "
                "scope — always call the app's own functions and read its own globals "
                "through `window.` (e.g. `window.generateQR()`, `window.buildContent()`), "
                "never as a bare identifier, or it will throw a ReferenceError even "
                "though the function really exists. When asserting on a string that "
                "should contain a literal backslash character, remember one backslash "
                "in source is written as two (`\\\\`) inside a JS string literal — "
                "double check backslash counts carefully. "
                "Exercise the real functions/events on the page (e.g., dispatch click "
                "events, set input .value, call an existing global function). Do not "
                "enable external resources, network access, or install packages, and "
                "do not reproduce the application algorithm in the test. jsdom is not "
                "a real browser: layout/canvas/visual behavior requires Playwright."
            ),
            solver_guidance=(
