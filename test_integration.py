...
class OrchestrationTests(unittest.TestCase):
    def test_all_adapters_reach_clean_replay(self):
        for (runtime, files), mode in itertools.product(FIXTURES.items(), ("pass", "zero_tests", "compile_error", "tampered_test", "candidate_inference_error")):
            with self.subTest(runtime=runtime, mode=mode), tempfile.TemporaryDirectory() as directory:
                ...
                with patch.dict("os.environ", env, clear=True), \
                     ...
                     patch.object(proof, "generate_candidate", return_value=([{"path": source, "content": files[source] + "\n"}], "repair"), side_effect=proof.InferenceError("Inference stopped") if mode == "candidate_inference_error" else None) as generate, \
                    ...
                    if mode == "pass":
                        ...
                        self.assertEqual(generate.call_count, {"zero_tests":3, "candidate_inference_error":1}.get(mode, 0))
                        continue
    ...
    def test_verifier_inference_failure_is_not_retried_by_execute(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "app.py").write_text("VALUE = 0\n")
            env = {"NEBIUS_API_KEY":"test", "NEBIUS_PROJECT_ID":"test",
                   "NEBIUS_MODEL":"test", "CONTREE_IMAGE":"image"}
            with patch.dict("os.environ", env, clear=True), \
                 patch.object(proof, "generate_regression_test", side_effect=proof.InferenceError("HTTP 401")) as generate, \
                 patch.object(proof, "create_sandbox_client") as sandbox:
                with self.assertRaises(proof.InferenceError):
                    proof.execute(root, proof.Issue(1, "Bug", "Expected value"), {"candidates": []})
                generate.assert_called_once()
                sandbox.assert_not_called()
                self.assertFalse((root / "test_patchproof_issue_1.py").exists())
